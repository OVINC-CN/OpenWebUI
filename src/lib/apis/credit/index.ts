...
// Model pricing API functions

export const getModelsPricing = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/credit/models/price`, {
		method: 'GET',
		headers: {
			Authorization: `Bearer ${token}`
		}
...
export const updateModelsPricing = async (token: string, pricing: Record<string, any>) => {
	let error = null;
...
export const fetchPricingFromOpenAI = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/credit/models/pricing/fetch`, {
		method: 'POST',
		headers: {
			Authorization: `Bearer ${token}`,
			Accept: 'application/json',
